...
class AppFocusDetector extends StatefulWidget {
  const AppFocusDetector({
    required this.child,
    this.onFocusGained,
    this.onFocusLost,
  ...
  @override
  AppFocusDetectorState createState() => AppFocusDetectorState();
}

class AppFocusDetectorState extends State<AppFocusDetector>
  ...
  void _notifyPlaneTransition(final AppLifecycleState state) {
    if (!_isWidgetVisible) {
      return;
    }
  ...
  void _notifyVisibilityStatusChange(final double newVisibleFraction) {
